...
func SetLogLevel(level string) {
	...
	for i, logLevelStr := range severityName {
		if logLevelStr == level {
			LogLevel = severity(int32(i))
			return
		}
	}
}
...
func (s *severity) enabled() bool {
	return severity(atomic.LoadInt32((*int32)(s))) >= LogLevel
}
...
func (l *loggingT) formatHeader(s severity, file string, line int) *buffer {
	...
	buf.tmp[19] = '.'
	buf.nDigits(3, 20, now.Nanosecond()/1000/1000, '0')
	buf.tmp[23] = ' '
	buf.tmp[24] = severityChar[s]
	buf.tmp[25] = ' '
...
func (l *loggingT) output(s severity, buf *buffer, file string, line int, alsoToStderr bool) {
	...
	data := buf.Bytes()
	if !flag.Parsed() {
		os.Stderr.Write([]byte("INFO: logging before flag.Parse: "))
...
// Schedulef logs to the INFO log.
// Arguments are handled in the manner of fmt.Printf; a newline is appended if missing.
func Schedulef(format string, args ...interface{}) {
	Debugf(format, args)
}
...
func Handlef(format string, args ...interface{}) {
	logging.printf(infoLog, " handle - "+format, args...)
}
